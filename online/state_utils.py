...
from data.generators import generate_online_sequence
from online.models import PlacementContext
...
def ensure_online_sequence(
    ...
    *,
    ...
    """
    if instance.online_items and instance.online_feasible is not None:
        return
    online_items, online_feasible, online_costs = generate_online_sequence(
        cfg,
        seed,
        start_id=len(instance.offline_items),
        horizon=horizon,
    )
    instance.online_items = online_items
    instance.online_feasible = online_feasible
    if online_costs.size:
        instance.costs.assign = np.vstack([instance.costs.assign, online_costs])
