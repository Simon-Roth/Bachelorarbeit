...
class OfflineMILP:
    ...
    def __init__(self, cfg: OfflineConfig, bins_df: pd.DataFrame, items_df: pd.DataFrame,
        ...
        if cfg.use_fallback and self.col_names[-1] == FALLBACK_COL:
    ...
    def validate(self, sol: Dict) -> Dict:
        ...
        # 3) fallback count
        num_fallback = int(assigned[FALLBACK_COL].sum()) if (self.cfg.use_fallback and FALLBACK_COL in assigned.columns) else 0
